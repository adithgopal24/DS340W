...
data_dir <- "C:/Users/malco/Downloads/plate_discipline_code-master/data/" 

# SPECIFY DIRECTORY TO SAVE MODEL
model_dir <- "C:/Users/malco/Downloads/plate_discipline_code-master/models/" 
...
swung_pitches$plate_x <- scales::rescale(swung_pitches$plate_x, to = c(-1,1), from = range(pitch_data$plate_x))
swung_pitches$plate_z <- scales::rescale(swung_pitches$plate_z, to = c(-1,1), from = range(pitch_data$plate_z))
swung_pitches$b_cwoba <- scales::rescale(swung_pitches$b_cwoba, to = c(-1,1), from = range(pitch_data$b_cwoba))
swung_pitches$p_cwoba <- scales::rescale(swung_pitches$p_cwoba, to = c(-1,1), from = range(pitch_data$p_cwoba))
